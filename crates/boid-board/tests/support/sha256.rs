//! SHA-256, hand-rolled, shared by every test that pins a constant against its content.
//!
//! `boid-board` is the dependency root of the workspace and takes no external dependencies
//! for its own code (D-0014). This is the one hash function the test suite needs, and it is
//! implemented here rather than pulled in, exactly as it was when `oracle_transcription.rs`
//! was its only consumer.
//!
//! Correctness is not assumed. [`sha256_implementation_matches_published_vectors`] checks
//! it against the NIST FIPS 180-2 examples, and that test travels with the implementation:
//! a pinning hash whose own correctness is unevidenced pins nothing.
